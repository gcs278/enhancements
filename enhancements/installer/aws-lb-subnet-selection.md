...
This use case won't require a new workflow, but instead by automatically solved by the [proposal](#proposal)
to reject untagged subnets in new installations.
...
- Maintain support for configuring the `default` IngressController subnets via customizing installer manifest.
...
2. Add the `spec.loadBalancer.platform.aws.classicLoadBalancer.subnets` and
   ...
   CRD to encode the default subnets for IngressControllers.
...
`kubernetes.io/cluster/unmanaged` tag to any subnets they wish to exclude this cluster installation.
...
fields will be added to the Ingress Config to encode the default subnets for user-created IngressControllers. While
earlier enhancements used the Ingress Config to also configure the `default` IngressController, it is more
straightforward for the installer to directly set the `default` IngressController subnets in its manifest. This
approach also prevents the Ingress Config API from becoming overloaded.
...
type AWSClassicLoadBalancerParameters struct {
    // subnets specifies the default subnets for user-created IngressControllers.
...
#### Setting Default IngressController subnets with BYO VPC during Installation Workflow
...
2. The OpenShift Installer populates the both the `default` IngressController and the `cluster` Ingress Config
   manifests with the IngressController-role subnets and installs the cluster into the VPC:
   ```yaml
...
A cluster admin wants to provide cluster-role subnets to use an existing VPC, but still have the AWS CCM
...
2. The OpenShift Installer installs the cluster into the VPC where the subnets exist, and populates the
   IngressConfig's `spec.loadBalancer.platform.aws.networkLoadBalancer.subnets` and
   `spec.loadBalancer.platform.aws.networkLoadBalancer.subnets` field.
...
### Enforcing IngressController-Role Subnets as a Subset of Cluster-Role Subnets Alternative

The installer could automatically determine IngressController-role subnets as a subset of the cluster-role subnets,
selecting only public or private cluster-role subnets based on whether the cluster is configured as internal
or external, as suggested in the closed bug [OCPBUGS-17432](https://issues.redhat.com/browse/OCPBUGS-17432).

This approach would result in OpenShift clusters completely circumventing the problematic AWS CCM subnet discovery
logic (introduced by [kubernetes/kubernetes#97431](https://github.com/kubernetes/kubernetes/pull/97431)) and
could be done without a new API field, using the existing `platform.aws.subnets`.

The installer would be updated to ensure that the appropriate subnets in `platform.aws.subnets`
automatically propagates to the Ingress Config's `spec.loadBalancer.platform.aws.classicLoadBalancer.subnets` and
`spec.loadBalancer.platform.aws.networkLoadBalancer.subnets` fields. This would make the `default` IngressController
subnets a subset of the provided cluster-role subnets.

While this would address the majority of customer use cases, this would invalidate the [Dedicated IngressController Subnets](#dedicated-ingresscontroller-subnets)
use case and could potentially be seen as a breaking change. However, the previous behavior is arguably invalid, with
limited evidence suggesting that customers currently rely on the AWS CCM selecting subnets lacking the
`kubernetes.io/cluster/<cluster-id>` tag. Furthermore, customers could effectively revert back to the original
behavior by using [custom installer manifests](https://github.com/openshift/installer/blob/master/docs/user/customization.md#kubernetes-customization-unvalidated).
